...
for t in tickers:
    ...
    dt = pd.DataFrame(data=historic_data,columns=['Date','Open','High','Low','Close','Volume'])
    dt.index = pd.to_datetime(dt['Date'],infer_datetime_format=True)
    dt = dt.drop(labels=['Date'],axis=1)
    dt['Adj Close'] = dt['Close']

    dt.to_csv('Nifty50 Select/' + t + ' ' + dtt.datetime.today().strftime('%m.%d.%Y') + '.csv')

    print('Finished getting data for %s ... ' %t)
